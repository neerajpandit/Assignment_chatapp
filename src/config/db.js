...
const pool = mysql.createPool({
  host: isProduction ? process.env.AWS_DB_HOST : process.env.DB_HOST || 'localhost',
  user: isProduction ? process.env.AWS_DB_USER : process.env.DB_USER || 'root', // Use the correct MySQL user
  database: isProduction ? process.env.AWS_DB_NAME : process.env.DB_NAME || 'chatapp', // Replace with your DB name
  password: isProduction ? process.env.AWS_DB_PASSWORD : process.env.DB_PASSWORD || 'Neeraj@1234',
  port: isProduction ? process.env.AWS_DB_PORT : process.env.DB_PORT || '3306', // MySQL default port is 3306
  waitForConnections: true, // Ensure to wait for connections to be available
